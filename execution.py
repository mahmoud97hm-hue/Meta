...
Owns:
  - _execute_smart_order (limit/market/FOK via bridge)
...
async def _execute_smart_order(symbol: str, is_buy: bool, lot: float,
    ...
    try:
        ...
        res = await client.create_pending_order(
            broker_symbol, action, lot, limit_price,
    ...
    try:
        ...
        if is_real:
            ...
            try:
                ...
                real_msg = "\n⚠️ لا يوجد اتصال mt5-bridge صالح — لم يتم فتح أي صفقة."
                is_real = False
...
async def _close_metaapi_trade(symbol: str, tid: str, sym_state: dict) -> bool:
    ...
    try:
        ...
        from telegram_ui import send_tg_msg
        await send_tg_msg(f"🛑 <b>تعذّر إغلاق صفقة {symbol} ({tid}):</b> لا يوجد اتصال mt5-bridge.")
        return False
    try:
        result = await client.close_position(tid)
        if not result.get('success'):
            from telegram_ui import send_tg_msg
            await send_tg_msg(f"⚠️ <b>فشل إغلاق صفقة {symbol} ({tid}):</b> {result.get('error', 'unknown')} يرجى التحقق يدوياً من الحساب.")
            return False
        for _ in range(25):
            positions = await client.get_positions()
