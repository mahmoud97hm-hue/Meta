...
<<<<<<< HEAD
        [{'text': '🔌 فحص حالة حساب MT5 Bridge', 'callback_data': 'check_metaapi_status'}],
=======
...
async def check_metaapi_status_command(chat_id: int):
    from mt5_bridge_client import get_bridge_client
    client = get_bridge_client()
    await send_tg_msg("⏳ جاري فحص حالة الحساب من MT5 Bridge...")
    try:
        health = await client.health_check()
        conn_state = health.get('state', 'unknown')
        msg = f"<b>حالة الحساب (MT5 Bridge)</b>\nحالة الاتصال: {conn_state}\n\n"
        if conn_state == 'connected':
            acc = await client.get_account_info()
            msg += f"<b>الرصيد:</b> {acc.get('balance', 0):.2f}\n"
            msg += f"<b>الاكويتي:</b> {acc.get('equity', 0):.2f}\n\n"
            positions = await client.get_positions()
            msg += f"<b>الصفقات المفتوحة:</b> {len(positions)}\n"
            for p in positions:
                msg += f"🔸 {p.get('symbol', '?')} | {p.get('type', '?')} | Vol: {p.get('volume', 0)} | Profit: {p.get('profit', 0):.2f}\n"
        else:
            msg += "⚠️ الحساب غير متصل حالياً لجلب تفاصيل الرصيد والصفقات."
        await send_tg_msg(msg)
    except Exception as e:
        await send_tg_msg(f"❌ خطأ في الاتصال بـ MT5 Bridge:\n{html_mod.escape(str(e))}")
=======
